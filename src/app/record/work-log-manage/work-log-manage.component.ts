...
export class WorkLogManageComponent implements OnInit {
  ...
  ngOnInit() {
    this.url = SystemConstant.WORK_LOG_PAGE_LIST;
  }
  /**
   * 查询
   */
  ...
  */
  addWorkLog() {
    const modalRef = this.ngbModal.open(WorkLogEditComponent);
  ...
  editWorkLog(id) {
    // 获取数据
    this.httpService.get(SystemConstant.WORK_LOG_DETAIL + '/' + id).subscribe({
      next: (data) => {
        this.openEdit(data);
      },
      error: (err) => {
  ...
  openEdit(myData) {
    const modalRef = this.ngbModal.open(WorkLogEditComponent);
    modalRef.componentInstance.recordWorkLogInputRequest = myData;
    modalRef.result.then(
      (result) => {
        if (result === 'success') {
          this.search();
  ...
  delWorkLog(id) {
    ...
    this.modalService.confirm(confirmCfg).then(
      () => {
        this.httpService.post(SystemConstant.WORK_LOG_DEL + '/' + id , {} ).subscribe({
          next: (data) => {
